...
class _LicenseScreenState extends State<LicenseScreen> {
  ...
  Future<void> _activateLicense() async {
    ...
    if (license.isEmpty) {
      setState(() {
        _message = 'Por favor, insira uma licença válida';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success' || data['valid'] == true) {
          ...
          setState(() {
            _message = '✅ Licença ativada com sucesso!';
          ...
          setState(() {
            _message = '❌ Licença inválida. Tente novamente.';
            _messageColor = Colors.red;
          });
        }
      } else {
        setState(() {
          _message = '❌ Erro no servidor. Tente novamente.';
          _messageColor = Colors.red;
        });
      }
    } catch (e) {
      setState(() {
        _message = '❌ Erro de conexão. Verifique sua internet.';
        _messageColor = Colors.red;
      });
    } finally {
      setState(() => _isLoading = false);
  ...
  Future<void> _saveConfigFile() async {
    try {
      ...
      setState(() {
        _message = '❌ Erro ao salvar arquivo de configuração';
        _messageColor = Colors.red;
      });
    }
  }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Erro ao excluir arquivo de configuração'),
